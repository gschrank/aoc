...
for (i=0; i<password.length; i++) {
    var res = (password[i].split(" "))
    word = (res[2])
    letter = res[1].match(/\w/)
    number = res[0]
    var y = new RegExp(letter, "g")
        if (word.match(y)) {
            z = word.match(y).length
            if ((z >= number.split("-")[0] && z <= number.split("-")[1]) === true) {
                result += 1 
            }
    }
}
